                ...
                <div className="bg-gray-900/50 p-4 rounded-lg mb-6 text-center border border-amber-500/50">
                    ...
                    {hasStartAge ? (
                        <p className="text-lg md:text-xl text-white leading-relaxed">
                            <span className="font-bold">{projection.startAge}</span>세부터 매월 <span className="font-bold text-green-400">{formatCurrency(data.targetMonthlyInvestment, '만원')}</span> 투자시, <span className="font-bold">{projection.startAge! + projection.periodYears}</span>세에<br/>(예상) 총자산 <span className="font-bold text-cyan-400">{formatCurrencyShort(projection.targetAssets)}</span> <span className="text-base text-gray-400">(현재 가치: {formatCurrencyShort(projection.inflationAdjustedTargetAssets)})</span>,<br/>월 분배금 <span className="font-bold text-indigo-400">{formatCurrencyShort(projection.finalMonthlyDividend)}</span> <span className="text-base text-gray-400">(현재 가치: {formatCurrencyShort(projection.inflationAdjustedMonthlyDividend)})</span> 기대
                        </p>
                    ) : (
                        <p className="text-lg md:text-xl text-white leading-relaxed">
                            ...
                            월 분배금 <span className="font-bold text-indigo-400">{formatCurrencyShort(projection.finalMonthlyDividend)}</span> <span className="text-base text-gray-400">(현재 가치: {formatCurrencyShort(projection.inflationAdjustedMonthlyDividend)})</span>
                        </p>
                    )}
                </div>
            )}
                ...
                <div className="bg-gray-900/50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                         ...
                         <div>
                            <p className="text-gray-400">월 수령 분배금 (세전, 직접입력)</p>
                            <div className="flex items-center bg-gray-700 rounded-lg mt-1">
                                ...
                                return (
                                    <div key={ticker}>
                                        <div className="flex justify-between items-baseline text-sm mb-1">
                                            <span className="font-bold text-gray-300">{etfData[ticker]?.name || ticker} ({ticker})</span>
