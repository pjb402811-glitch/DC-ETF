...
const SimulatorForm: React.FC<SimulatorFormProps> = ({ onSubmit }) => {
    const [goalType, setGoalType] = useState<'dividend' | 'asset' | 'investment'>('asset');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const initialPrincipalVal = cleanAndParse(initialPrincipal) * 10000; // 만원 -> 원
        ...
        onSubmit(goal, initialPrincipalVal, currentAgeVal, investmentPeriodVal, riskProfile, investmentTheme, inflationRateVal);
    };

    return (
        <section className="bg-gray-800 p-6 rounded-2xl shadow-lg mb-12">
            <h2 className="text-2xl font-semibold text-green-400 mb-6 text-center">나의 은퇴 계획 입력하기</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="investment-theme" value={investmentTheme} onChange={e => setInvestmentTheme(e.target.value as InvestmentTheme)} className="w-full bg-gray-700 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
                            ...
                            <option value="stable-income">안정 인컴 중심 (안전자산 70% 이상)</option>
                            <option value="balanced-growth">균형 성장 (위험자산 50% 내외)</option>
                            <option value="max-growth-70">성장 집중 (위험자산 70%)</option>
                            <option value="tdf-focused">TDF 중심 (자동 자산배분)</option>
                        </select>
                    </div>
                     <div style={{ opacity: isRiskProfileDisabled ? 0.5 : 1 }}>
                        ...
                        <select id="risk-profile" value={riskProfile} onChange={e => setRiskProfile(e.target.value as RiskProfile)} className="w-full bg-gray-700 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" disabled={isRiskProfileDisabled}>
                            <option value="conservative">안정 추구형 (위험자산 30%)</option>
                            <option value="balanced">균형 성장형 (위험자산 50%)</option>
                            <option value="aggressive">적극 성장형 (위험자산 70%)</option>
                        </select>
                    </div>
                    ...
                    <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div className={`p-4 rounded-xl transition-all duration-300 ${goalType === 'dividend' ? 'bg-gray-900/50 ring-2 ring-green-500' : ''}`} style={{opacity: goalType === 'dividend' ? 1 : 0.5}}>
                            <label htmlFor="target-dividend" className="block text-sm font-medium text-gray-300 mb-2">목표 월 분배금 (만원)</label>
                    ...
                    <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-4 gap-6">
                         <div>
                            <label htmlFor="initial-principal" className="block text-sm font-medium text-gray-300 mb-2">현재 퇴직연금 잔고</label>
                            <div className="flex items-center bg-gray-700 rounded-lg">
                                <input type="text" inputMode="numeric" id="initial-principal" value={initialPrincipal} onChange={handleNumericInputChange(setInitialPrincipal)} className="w-full bg-transparent p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="예: 1000" />
                                <span className="text-gray-400 pr-4 flex-shrink-0">만원</span>
                            </div>
                        </div>
                        <div>
                            <label htmlFor="current-age" className="block text-sm font-medium text-gray-300 mb-2">현재 나이 (세)</label>
                            <div className="flex items-center bg-gray-700 rounded-lg">
                                <input type="number" id="current-age" value={currentAge} onChange={e => setCurrentAge(e.target.value)} className="w-full bg-transparent p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="예: 30" />
                        ...
                        <div>
                            ...
                            <div className="flex items-center bg-gray-700 rounded-lg">
                                <input type="number" id="investment-period" value={investmentPeriod} onChange={e => setInvestmentPeriod(e.target.value)} className="w-full bg-transparent p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="예: 25" />
